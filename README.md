...
To replicate the results in Table 4 of the paper, run the notebook [Table 4.ipynb](scripts/Table%204.ipynb). The required pickle data for this replication is generated using the source codes available in the [source code](src/%20m%20=%205/) folder. To comply with data privacy requirements, we cannot share the actual hospital demand data used in the case study. Instead, the provided code utilizes simulated demand data based on the fitted negative binomial distributions reported in the paper.
