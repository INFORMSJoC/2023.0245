...
# Summarize daily counts of issues for the 'ML' site
tdemdata <- DataPLT_ML %>%
  filter(year(IssueDT) >= 2015 & !is.na(IssueDT)) %>%
  group_by(Site, IssueDT) %>%
...
tdemdata$Site[is.na(tdemdata$Site)] <- "ML"

# Rename the 'ML' site for better readability
levels(tdemdata$Site)[levels(tdemdata$Site) == "ML"] <- "Hamilton General Hospital"
...
ggsave("Figure 11.pdf", width = 14, height = 7.5, units = "in")
